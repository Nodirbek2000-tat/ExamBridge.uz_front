...
import { ChevronLeft, Mic, Star, MessageSquare, Loader2 } from 'lucide-react'
...
export default function IELTSSpeakingReview() {
  const { responseId } = useParams()
  const navigate = useNavigate()

  const { data, isLoading, isError } = useQuery({
    queryKey: ['speaking-review', responseId],
    queryFn: () => api.get(`/ielts/speaking/review/${responseId}/`).then(r => r.data),
    enabled: !!responseId,
  })
  ...
  return (
    <div className="space-y-5 max-w-2xl mx-auto pb-8">
      ...
      <div className="flex items-center gap-3">
        ...
        <div>
          <h1 className="font-black text-gray-900 text-lg leading-tight">{data.task_title}</h1>
          <p className="text-xs text-gray-400">Part {data.task_part} · Speaking Review</p>
        </div>
      </div>
      ...
      {data.transcripts?.length > 0 && (
        <div className="bg-white rounded-2xl border border-gray-100 p-5">
          <p className="font-bold text-gray-800 mb-3 flex items-center gap-2">
            <MessageSquare size={16} className="text-gray-400" /> Transcripts
          </p>
          <div className="space-y-3">
            {data.transcripts.map((t, i) => (
              <div key={i} className="bg-gray-50 rounded-xl p-3">
                {t.question && (
                  <p className="text-xs text-gray-500 font-semibold mb-1">Q: {t.question}</p>
                )}
                <p className="text-sm text-gray-700 leading-relaxed">{t.transcript || t.text || String(t)}</p>
              </div>
            ))}
          </div>
        </div>
