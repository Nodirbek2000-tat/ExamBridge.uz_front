...
export default function IELTSListeningList({ accentBtn = 'bg-sky-500 hover:bg-sky-600' } = {}) {
  ...
  const { data, isLoading } = useQuery({
    ...
    queryFn: () => api.get('/ielts/listening/').then((r) => r.data),
  })
  ...
  const items = useMemo(() => {
    const mocks = (data?.mocks || []).map((m) => ({
      ...m,
      itemType: 'MOCK',
      count: m.section_count || m.sections?.length || 1,
    ...
    const filtered = [...mocks, ...practices].filter((item) => {
      ...
      const matchSections = sections === 'ALL' || String(item.count) === sections
      return matchSearch && matchLevel && matchType && matchSections
    })
    return [...filtered.filter(i => !i.is_premium), ...filtered.filter(i => i.is_premium)]
  }, [data, search, level, type, sections])
  ...
  const handleStart = async (item) => {
    ...
    try {
      if (item.itemType === 'MOCK') {
        ...
        const first = parts[0]
        if (!first) return
        const allIds = parts.map(p => p.id).join(',')
        const res = await api.post(`/ielts/listening/mock/${item.id}/start/`)
        navigate(`/exam/ielts/listening/${res.data.attempt_id}?section=${first.id}&parts=${allIds}&title=${encodeURIComponent(item.title)}`)
      } else {
        const res = await api.post(`/ielts/listening/${item.id}/start/`)
        navigate(`/exam/ielts/listening/${res.data.attempt_id}?section=${item.id}&title=${encodeURIComponent(item.title)}`)
      }
    } finally {
      setStarting(null)
    }
                ...
                >
                  <div className="min-w-0 flex-1">
                    ...
                    <p className="text-sm text-gray-500 mt-1">
                      {item.count} section{item.count > 1 ? 's' : ''} - {item.difficulty || 'MEDIUM'} Level
                    </p>
