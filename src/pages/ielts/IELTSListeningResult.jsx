...
import { CheckCircle2, XCircle, RotateCcw, ChevronLeft, Home, ListChecks, Headphones } from 'lucide-react'
...
export default function IELTSListeningResult() {
  ...
  const title = decodeURIComponent(searchParams.get('title') || 'Listening')
  const sectionId = Number(searchParams.get('section') || 0)
  const part = searchParams.get('part') || ''
  const partQuery = part ? `&part=${encodeURIComponent(part)}` : ''
    ...
    return (
      <div className="h-full flex flex-col bg-white">
        <div className="h-14 px-4 flex items-center gap-3 bg-white border-b border-sky-100">
          <button type="button" onClick={() => navigate('/app/ielts/listening')}
        ...
        <div className="flex-1 p-4 max-w-3xl w-full mx-auto">
          <div className="bg-white border border-gray-200 rounded-2xl p-5">
            ...
            <p className="text-sm text-gray-500 mt-1">
              Sahifani yangilasangiz, natija yo'qoladi. Ro'yxatga qayting yoki testni qayta yakunlang.
            </p>
            <button type="button" onClick={() => navigate('/app/ielts/listening')}
  ...
  const handleTryAgain = async () => {
    ...
    setRetrying(true)
    try {
      const res = await api.post(`/ielts/listening/${sectionId}/start/`)
      navigate(`/exam/ielts/listening/${res.data.attempt_id}?section=${sectionId}${partQuery}&title=${encodeURIComponent(title)}`)
    } finally {
      setRetrying(false)
    }
  }
      ...
      <div className="flex-1 overflow-y-auto p-4">
        <div className="max-w-3xl mx-auto w-full space-y-4">
          ...
          <div className="bg-white border border-sky-100 rounded-2xl p-6">
            ...
            <div className="mt-6 rounded-xl border border-sky-100 bg-sky-50/40 px-5 py-4 flex items-center justify-between">
              ...
              <div className="text-right">
                ...
                {summary.isFull ? (
                  <p className={`text-xl font-bold mt-1 ${getBandColor(summary.band)}`}>
            ...
            {summary.isFull && (
              <div className="mt-3 grid grid-cols-3 gap-3">
                {[
                  { label: 'Band Score', value: Number(summary.band).toFixed(1), color: getBandColor(summary.band) },
          ...
          <div className="bg-white border border-gray-200 rounded-2xl p-4">
            ...
            <button type="button" onClick={handleFeedbackSubmit} disabled={!feedbackText.trim()}
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-3 pb-4">
            ...
            <button type="button" onClick={() => navigate('/app/ielts/listening')}
              className="h-14 rounded-xl bg-gray-100 text-gray-700 font-semibold flex items-center justify-center gap-2 hover:bg-gray-200 transition">
              <Headphones size={16} /> Listening tests
            </button>
            <button type="button"
              onClick={() => navigate(`/exam/ielts/listening/${attemptId}?section=${sectionId}${partQuery}&title=${encodeURIComponent(title)}&mode=review`)}
            ...
            <button type="button" onClick={handleTryAgain} disabled={retrying}
